...
function AdminDashboard() {
    ...
    async function handleEventSubmit(e) {
        ...
        setEventData({ title: '', description: '', location: '', date: '', capacity: '', maxVolunteers: '' });
    }
    ...
    async function handleToggleEvent(eventId) {
        ...
        try {
            const res = await fetch(
                `http://localhost:3001/api/events/${eventId}/volunteer-applications`,
                { credentials: 'include' }
            );
            if (res.ok) {
                const data = await res.json();
                setApplications(prev => ({ ...prev, [eventId]: data.applications || [] }));
            }
        } catch (err) { console.error(err); }
        finally { setLoadingApps(prev => ({ ...prev, [eventId]: false })); }
    }
    ...
    async function handleStatusUpdate(eventId, applicationId, newStatus) {
        ...
        try {
            const res = await fetch(
                `http://localhost:3001/api/events/${eventId}/volunteer/${applicationId}/status`,
                {
            ...
            if (res.ok) {
                ...
                if (newStatus === 'rejected') {
                    setEvents(prev => prev.map(e =>
                        e._id === eventId ? { ...e, volunteersRemaining: (e.volunteersRemaining || 0) + 1 } : e
                    ));
                }
            }
        } catch (err) { console.error(err); }
    ...
    return (
        <div className="dashboard-page">
            ...
            <div className="dashboard-grid">
                ...
                <div className="stat-card">
                    <div className="stat-icon"><HandHelping size={24} /></div>
                    <div className="stat-content">
            ...
            <div className="dashboard-grid wide">
                ...
                <div className="dashboard-section">
                    ...
                    <div className="quick-actions">
                        ...
                        <button type="button" className="secondary-btn" onClick={() => setShowEventModal(true)}>Create Event</button>
                    </div>
                </div>
                ...
                <div className="dashboard-section">
                    ...
                    <ul className="section-list">
                        {stats?.recentNotices?.map(n => <li key={n._id}>{n.title}</li>)}
                        {(!stats?.recentNotices || stats.recentNotices.length === 0) && (
                            <li className="empty-msg">No notices yet.</li>
                        )}
                    </ul>
                </div>
                ...
                <div className="dashboard-section">
                    ...
                    <ul className="section-list">
                        {stats?.recentEvents?.map(e => <li key={e._id}>{e.title}</li>)}
                        {(!stats?.recentEvents || stats.recentEvents.length === 0) && (
                            <li className="empty-msg">No events yet.</li>
                        )}
                    </ul>
                </div>
            </div>
            ...
            <div className="dashboard-section vol-admin-section">
                <div className="section-header">
                    <h2 className="section-title">
                        ...
                        {pendingTotal > 0 && (
                            <span className="tab-badge" style={{ marginLeft: 8 }}>{pendingTotal}</span>
                        )}
                    </h2>
                </div>
                        ...
                        {events.map(ev => {
                            ...
                            return (
                                <div key={ev._id} className="vol-admin-event">
                                    {/* Accordion header */}
                                    <button
                                        className="vol-admin-event-header"
                                    ...
                                    >
                                        <div className="vol-admin-event-info">
                                            ...
                                            <span className="vol-admin-event-meta">
                                                ...
                                                {pendingCount > 0 && (
                                                    <span className="tab-badge" style={{ marginLeft: 8 }}>{pendingCount} pending</span>
                                                )}
                                            </span>
                                        </div>
                                    ...
                                    {isOpen && (
                                        <div className="vol-admin-apps">
                                            ...
                                            {!loading && apps.length === 0 && (
                                                <p className="empty-msg">No applications yet for this event.</p>
                                            )}
                                                    ...
                                                    <div className="vol-admin-app-actions">
                                                        ...
                                                        {app.status !== 'approved' && (
                                                            <button
                                                                ...
                                                                onClick={() => handleStatusUpdate(ev._id, app._id, 'approved')}
                                                            >
                                                                <Check size={14} />
                                                            </button>
                                                        )}
                                                        {app.status !== 'rejected' && (
                                                            <button
                                                                className="vol-action-btn reject"
                                                                title="Reject"
                                                                disabled={actionLoading === app._id}
                                                                onClick={() => handleStatusUpdate(ev._id, app._id, 'rejected')}
                                                            >
                                                                <X size={14} />
                                                            </button>
                                                        )}
